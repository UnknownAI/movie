...
В подкатологе ``` /src ``` хранятcя файлы:
 ...
 - ``` index.js ```, который является точкой входа JavaScript; в этом файле определяется элемент страницы index.html, куда будет отрисовываться основной компонент (`<App />`) 
...
Стили приложения хранятся в папке ``` /scss ```
